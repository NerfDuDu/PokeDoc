 # Copyright (C) 2024 PokeDoc / NerfDuDu
 #
 # Ce programme est un logiciel libre : vous pouvez le redistribuer et/ou le modifier
 # selon les termes de la licence GNU Lesser General Public License (LGPL) version 3,
 ...
 # Voir la licence GNU LGPL v3 pour plus de détails.
 #
 # Vous devriez avoir reçu une copie de la licence GNU LGPL v3 avec ce programme.
 # Sinon, consultez <https://www.gnu.org/licenses/>.
...
class SAKA_DHO(QWidget):
    def __init__(self, page_index, total_pages):
        super().__init__()
        ...
        self.barre.textChanged.connect(self.rechercher_pokemon)  # Connecter la barre de recherche à la fonction de recherche
    ...
    def afficher_pokemons(self, pokemons=None):
        """ Affiche les Pokémon dans la grille """
        if pokemons is None:
        ...
        for i in range(5):
            for j in range(4):
                ...
                if num < len(pokemons):  # Vérifier si le Pokémon existe
    ...
    def rechercher_pokemon(self):
        ...
        texte_recherche = self.barre.text().strip().lower()
        if texte_recherche:
            pokemons_filtres = [pokemon for pokemon in self.pokedex if texte_recherche in pokemon["name"]["french"].lower()]
